...
public class DispatcherServlet extends HttpServlet {
    ...
    public DispatcherServlet() throws FileNotFoundException, IOException, ClassNotFoundException, NoSuchMethodException, SecurityException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
    	...
    	String filename = DispatcherServlet.class.getResource("/url-command.properties").getPath(); // /target/class밑에 있는 url-command.properties의 절대경로
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(viewName != null ) {
			if(viewName.startsWith("redirect:")) {
				...
				// forward
				String prefiex = "/WEB-INF/views/";
				String suffix = ".jsp";
				viewName = prefiex + viewName + suffix;	// /WEB-INF/views + student/studentList + .jsp
				request.getRequestDispatcher(viewName).forward(request, response);
			}
		}
		
		// viewName이 null인 경우 컨트롤러에서 응답메세지를 직접 작성한 경우 (비동기) 아무것도 하지 않음
